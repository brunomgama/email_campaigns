...
export function EditAudienceModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!audienceId || !validateForm()) {
      return
    }

    setIsSubmitting(true)
    try {
      const requestData: UpdateAudienceRequest = {
      ...
      await audienceApi.update(audienceId, requestData)
      ...
      let errorMessage = "Failed to update audience"
      ...
      <div className="relative z-10 w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden">
        <div className="bg-background border rounded-lg shadow-lg flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b">
            <div>
              <h2 className="text-lg font-semibold">Edit Audience</h2>
              <p className="text-sm text-muted-foreground">Update the audience information below.</p>
            </div>
            <Button
              variant="ghost"
              size="icon"
